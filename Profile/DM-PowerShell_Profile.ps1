...
if (Test-Path "$($KeyPath)\$($env:AdminUPN).cred") {
    ...
    # Dot source the function to enable the Get-StoredCredential function from $KeyPath (should probably be turned into a module)
...
Write-Host -ForegroundColor 'White' "Running as $Env:Username with profile path: $KeyPath `nDomain admin credentials loaded:" $([bool]$AdminCredential) "`n"
