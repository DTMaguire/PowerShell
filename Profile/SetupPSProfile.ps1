...
#
# Run script in an elevated PowerShell session with your normal user account
# This does mean local admin access for your standard account is required - at least for this setup
#
# This script will:
...
do {
    ...
    $Script:AdminName = (Read-Host "Enter the username of your Domain Admin account")
...
if (!((Read-Host -Prompt "`nSetup stored credentials function now? (Y/n)") -eq 'N')) {
    ...
    # Set up the webclient with proxy credentials and attempt to download the file 
    try {
        $WebClient = New-Object System.Net.WebClient
        $WebClient.Proxy.Credentials = [System.Net.CredentialCache]::DefaultNetworkCredentials
        $WebClient.DownloadFile($StoredFunctionURL,"$ProfilePath\$ZipFile")
        Write-Host -ForegroundColor 'Green' "`nFile downloaded with default network credentials.`n"
    }
    catch {
        # Try again with different credentials...
        try {
            $WebClient.Proxy.Credentials = (Get-Credential -Message "Please enter valid proxy credentials")
            $WebClient.DownloadFile($StoredFunctionURL,"$ProfilePath\$ZipFile")
            Write-Host -ForegroundColor 'Cyan' "`nFile downloaded with specified credentials.`n"   
        }
        catch {
            Write-Error -Message "`nUnable to download `'$ZipFile`' file from GitHub. `
            You can download the file manually to your profile path and re-run the setup script.`n"
        }
    }
    ...
    if (Test-Path -Path "$ProfilePath\$ZipFile" -PathType Leaf) {
        ...
        Write-Warning `
            "Unable to expand archive - you can download the file manually and extract .ps1 file to $ProfilePath`n"
    }
