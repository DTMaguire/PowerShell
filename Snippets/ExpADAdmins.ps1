...
$ExpAdmins = @(Get-Content -Path D:\Scripts\Output\ExpADAdmins.txt)

ForEach ($Admin in $ExpAdmins) {
    Get-ADUser -Identity $Admin -Properties * | 
    Select-Object -Property Name,AccountExpirationDate,AccountLockoutTime,BadLogonCount,Enabled,`
    LastBadPasswordAttempt,LastLogonDate,LockedOut,lockoutTime,PasswordExpired,PasswordLastSet,PasswordNeverExpires |
